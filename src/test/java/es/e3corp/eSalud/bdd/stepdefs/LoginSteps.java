...
public class LoginSteps {
	...
	public void abrimos_el_navegador_e_iniciamos_la_aplicacion() {
	       ...
	       driver.get("http://esalud.herokuapp.com/auth/login");
	}
	...
	public void accedemos_a_la_aplicacionla_aplicacion() throws InterruptedException {
		...
		if(a.get(0).get("testCase")== "CASE 1") { 
			...
			assertEquals("http://esalud.herokuapp.com/admin",expectedUrl);
			
		}else if(a.get(0).get("testCase")== "CASE 2"){
			
			
			
			assertEquals("http://esalud.herokuapp.com/listado-citas",expectedUrl);
			
		}else if(a.get(0).get("testCase")== "CASE 3") {
			
			
			
			assertEquals("http://esalud.herokuapp.com/sidenavAdmin",expectedUrl);
			
		}else {
			
			
			
			assertEquals("http://esalud.herokuapp.com/auth/login",expectedUrl);
