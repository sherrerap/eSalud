...
public class Modificar_CitasSteps {
	...
	public void el_usuario_paciente_modifica_los_datos_de_una_cita(io.cucumber.datatable.DataTable dataTable) {
		/*
		 * SUPUESTO: a la hora de modificar cita primero debes indicar como usuario
		 * que quieres modificar una cita seleccionando Modificar cita y posteriormente
		...
		cita.setTipo(a.get(0).get("tipo"));
		cita.setFecha(a.get(0).get("fecha"));
		cita.setHora(a.get(0).get("hora"));
		cita.setCentro(a.get(0).get("centro"));
		cita.setMedico(a.get(0).get("médico"));

		driver.findElement(By.xpath("//input[@placeholder='form_paciente']")).clear();
		driver.findElement(By.xpath("//input[@placeholder='form_paciente']")).sendKeys(cita.getPaciente());
		
		driver.findElement(By.xpath("//input[@placeholder='form_tipo']")).clear();
		driver.findElement(By.xpath("//input[@placeholder='form_tipo']")).sendKeys(cita.getTipo());
		
		driver.findElement(By.xpath("//input[@placeholder='form_fecha']")).clear();
		driver.findElement(By.xpath("//input[@placeholder='form_fecha']")).sendKeys(cita.getFecha());
		
		driver.findElement(By.xpath("//input[@placeholder='form_hora']")).clear();
		driver.findElement(By.xpath("//input[@placeholder='form_hora']")).sendKeys(cita.getHora());
		
		driver.findElement(By.xpath("//input[@placeholder='form_centro']")).clear();
		driver.findElement(By.xpath("//input[@placeholder='form_centro']")).sendKeys(cita.getCentro());
		
		driver.findElement(By.xpath("//input[@placeholder='form_medico']")).clear();
		driver.findElement(By.xpath("//input[@placeholder='form_médico']")).sendKeys(cita.getMedico());*/
	...
	public void se_modifica_la_cita_y_se_sustituye_en_la_base_de_datos(String string) {
		...
		case "No tipo presente":
			assertTrue(cita == null);
			break;
		case "No fecha presente":
			assertTrue(cita == null);
			break;
		case "No hora presente":
			assertTrue(cita == null);
			break;
		case "No centro presente":
			assertTrue(cita == null);
			break;
		case "No medico presente":
			assertTrue(cita == null);
			break;
		case "Fecha incorrecta":
			assertTrue(cita == null);
			break;
		case "Hora incorrecta":
			assertTrue(cita == null);
			break;
		case "Paciente inexistente":
			assertEquals(usuarioRepository.findByDniAndContraseña(a.get(0).get("paciente"), a.get(0).get("contraseña")),
					null);
			break;
		case "En el resto de casos":
			citasRepository.updateCita(cita);
			assertEquals(cita.getId(), citasRepository.findById(cita.getId).getId());
			citasRepository.deleteCita(cita.getId());
			break;
		}*/
		driver.close();
	}
